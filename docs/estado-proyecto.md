...
- **Sesión y login (Auth.js JWT)**: el callback `jwt` revalida el usuario contra la BD en cada petición (si se desactiva, su token deja de valer en la siguiente). Ante un **fallo de conexión** (errores `P1xxx`/inicialización de Prisma) se usa un mensaje **genérico** ("No se puede conectar con el servidor…", sin filtrar tipo de BD ni infra) y no se desloguea por un corte transitorio (se conserva el token); cualquier otro error de Prisma se re-lanza para no ocultar bugs de esquema. En el **login**, `authorize` distingue credenciales inválidas (`CredentialsSignin` → "Credenciales no válidas") de fallo de conexión (resto de `AuthError` → mensaje genérico en la propia pantalla de login, vía `?error=conexion`). Helper y mensaje centralizados en `src/auth.ts` (`isConnectionError`, `CONNECTION_ERROR_MESSAGE`). (v1.11.2)
